      ...
      <DialogContent className="w-[95vw] max-w-md mx-auto rounded-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {(isFromDayToday || isToDayToday) && (
            <div className="flex items-start space-x-2 p-2 sm:p-3 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-xs sm:text-sm flex items-center flex-wrap gap-1">
                        <span className="truncate">{daysOfWeek[fromDay]}</span>
                        {isFromDayToday && (
                          <Badge variant="outline" className="text-xs px-1 py-0">Today</Badge>
                        )}
                      </div>
                      <div className="text-xs text-gray-600 truncate">Currently: {fromDayName}</div>
                    </div>
                  </div>
                  <ArrowLeftRight className="h-4 w-4 text-gray-400 flex-shrink-0 ml-2" />
                </div>
              ...
              <CardContent className="p-2 sm:p-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-xs sm:text-sm flex items-center flex-wrap gap-1">
                        <span className="truncate">{daysOfWeek[toDay]}</span>
                        {isToDayToday && (
                          <Badge variant="outline" className="text-xs px-1 py-0">Today</Badge>
                        )}
                      </div>
                      <div className="text-xs text-gray-600 truncate">Currently: {toDayName}</div>
                    </div>
                  </div>
                  <ArrowLeftRight className="h-4 w-4 text-gray-400 flex-shrink-0 ml-2" />
                </div>
